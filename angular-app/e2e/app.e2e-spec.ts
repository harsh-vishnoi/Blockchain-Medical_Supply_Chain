...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be medicalsupply-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('medicalsupply-network@0.0.1.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('MedicalStore component should be loadable',() => {
      page.navigateTo('/MedicalStore');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('MedicalStore');
      });
    });

    it('MedicalStore table should have 3 columns',() => {
      page.navigateTo('/MedicalStore');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(3); // Addition of 1 for 'Action' column
      });
